...
public class BreadcrumbItemTagHelper : TagHelper
{
    ...
    public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
    {
        ...
        if (LastActiveLink)
        {
            ...
            output.TagName = "li";
            output.Attributes.SetAttribute("class", "cursor-default text-stone-500 dark:text-stone-400");
            output.Content.SetHtmlContent(cc.GetContent());
            return;
        }
        ...
        {
            var cc = await output.GetChildContentAsync();
            output.PostElement.SetHtmlContent($"<li class=\"flex\">\n<iconify-icon icon=\"lucide:dot\" width=\"26\" height=\"26\"></iconify-icon>\n</li>");
            output.TagName = "li";
            output.Attributes.SetAttribute("class", "cursor-default");
            output.Content.SetHtmlContent(cc.GetContent());
            return;
        }
        ...
        output.PostElement.SetHtmlContent("<li class=\"flex\">\n<iconify-icon icon=\"lucide:dot\" width=\"26\" height=\"26\"></iconify-icon>\n</li>");
        output.TagName = "li";
        output.Attributes.SetAttribute("class", "cursor-pointer");
        output.Content.SetHtmlContent($"<a href=\"{url}\">\n{childContent.GetContent()}\n</a>");
    }
}
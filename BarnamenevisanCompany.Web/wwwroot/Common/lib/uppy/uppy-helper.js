...
function initializeChunkUpload() {
    $("[data-chunk-upload]").each(function () {
        const allowedTypesString = $(this).attr("data-allowed-types") || null;
        const bindInput = $(this).attr("data-bind");
        if (bindInput === null || bindInput === "" || bindInput === undefined) {
            return;
        }
        let allowedFileTypes = null;
        if (allowedTypesString !== null) {
            allowedFileTypes = allowedTypesString
                .split(",")
                .map(type => type.trim())
                .map(type => type.replace(/['"]+/g, ''));
        }
        $(this).attr("id", "chunk-upload-" + id);
        const locationToSave = $(this).attr("data-loc") || undefined;
        const suggestedFileName = $(this).attr("data-sfm") || undefined;
        ...
        const uppy = new Uppy({
            ...
            .use(Tus, {
                endpoint: '/chunked-files',
                chunkSize: 5242880, // 5 MB chunks
                retryDelays: [0, 1000, 3000, 5000]
            })
            .use(ImageEditor, {
                target: Dashboard, // Image Editor integrates into the Dashboard
                quality: 0.8,      // Set image quality for compression (0.8 is 80% quality)
                cropperOptions: {
                    viewMode: 1,   // Set cropper.js view mode
                    background: false, // Disable background grid of the cropper
                }
            });
        ...
        uppy.on('error', (error) => {
            console.error('Error with Uppy upload:', error);
        });
        ...
        uppy.on('file-added', (file) => {
            if (locationToSave !== undefined) {
                file.meta['location'] = locationToSave;
            }
            if (suggestedFileName !== undefined) {
                file.meta['suggestedFileName'] = suggestedFileName;
            }
            // Set the filename metadata if needed
        ...
        uppy.on('upload-success', (file, response) => {
            ...
            const suggestedFileName = file.meta.suggestedFileName;
            const extension = file.extension;
            const savedFileName = suggestedFileName + "." + extension;

            if (savedFileName) {
                $(`[name="${bindInput}"]`).val(savedFileName);
            } else {
...
window.initializeChunkUpload = initializeChunkUpload;
